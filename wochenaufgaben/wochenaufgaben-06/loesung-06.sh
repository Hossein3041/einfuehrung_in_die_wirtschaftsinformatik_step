...
for mmsi in $(cat mmsi-list.txt | cut -d "|" -f2); do
  ...
  grep "$mmsi" /home/step2022/data/rhodes.txt | grep -e "^[^|]*|1" -e "^[^|]*|3" |cut -d "|" -f 1,3,9,10 | sed "s/^/|/g"| sort -t "|" | nl -n ln | while IFS="|" read nr zeit mmsi long lati; do echo "<tr><td>$nr</td><td>$zeit</td><td>$mmsi</td><td>$long</td><td>$lati</td></tr>"; done >> schritt-01/$mmsi.html
  echo "</table>
  </body>
