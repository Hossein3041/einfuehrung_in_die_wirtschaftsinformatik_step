...
cat mmsi-name.txt| cut -d "|" -f1 > mmsi-liste.txt

rm -rf result
mkdir -p result

diemmsis=$(cat mmsi-liste.txt)
echo $diemmsis
...
<body>
<table>
<tr><th>Nr</th><th>MMSI</th><th>Name</th></tr>" > index.html
...
mkdir -p htmla htmlb
for i in $(cat mmsi-list.txt); do
 cat result/$mmsi| |sed "s/^/<tr><td>/g"| sed "s/|/<\/td><td>/g"| sed "s/$/<\/td><\/tr>/g" > htmla/$mmsi
 echo "<!doctype html><html><head><title>$mmsi</title><meta charset='utf-8'></head><body><table><tr><td>Nr</td><td>Zeitstempel</td><td>MMSI</td><td>Longitude</td><td>Latitude</td></tr>" > htmlb/$mmsi.html
cat htmla/$mmsi >> htmlb/$mmsi.html
echo "</table></body></html>" >> htmlb/$mmsi.html
